...
class Granger:
    ...
    _file_names = {0: "time_series.csv", 1: "transfer_data.json", 2: "R_data.json"}
    _packages = {
        "bi-variate": ["statsmodels", "vars", "lmtest", "VLTimeCausality", "NlinTS"],
    ...
    def __is_valid(self, test_type, ts_data):
        ...
        if isinstance(ts_data, TS_Sim.TS_Sim):
            ...
            raise Warning(f"Please provide the time series data as TS_Sim.TS_Sim and not {type(ts_data)}")
    ...
    def __init_res(self):
        tmp_dict = dict(
            zip(self._packages[self.test_type], np.repeat(np.nan, repeats=len(self._packages[self.test_type])))
        )
    ...
    def __n_comb(self):
        self._num_comb = {"bi-variate": self.ts.k - 1, "multivariate": 2 ** (self.ts.k - 1) - 1, "all_on_one": 1}
    ...
    def __ts_combination(self):
        ...
        if self.test_type == "bi-variate":
            self.ts_combinations = pd.DataFrame(
                [self.ts.ts_names[np.delete(np.arange(self.ts.k), i)] for i in np.arange(self.ts.k)]
            ).T
        elif self.test_type == "multivariate":
            self.ts_combinations = pd.DataFrame(
                [
                    [[*j] for j in mit.powerset(mit.unique_everseen(np.delete(self.ts.ts_names, t)))][1:]
        ...
        for l in np.arange(self.ts.p):
            ...
            for t in np.arange(self.ts.k):
                for r, i in enumerate(self.ts_combinations[t]):
                    self.statsm_gc_objects[l][r, t] = tmp_var.test_causality(
                        caused=self.ts.ts_names[t], causing=i, signif=self.sig_lvl
                    )
    ...
    def __run_partial_gc(self):
        ...
        for i, d in enumerate(self.gc_partial_pvalues):
            for ts in np.arange(self.ts.k):
                d[:, ts] = [
                    TB.Tools.partial_granger_causality(x=self.ts.df_ts.iloc[:, j], y=self.ts.df_ts.iloc[:, ts], p=i + 1)
                    for j in np.delete(np.arange(self.ts.k), ts)
                ]
    ...
    def __gc_transform_pvalues(self, p_values):
        ...
        s_sel = (
            np.array([np.delete(np.arange(self.ts.k), i) for i in np.arange(self.ts.k)])
            .flatten()
            .reshape(self.ts.k, -1)
        )
        if self.test_type == "bi-variate":
            for l in np.arange(gc_significant.shape[0]):
                # TODO check
                gc_res[l][
                    :, np.repeat(np.arange(self.ts.k), self.ts.k - 1).reshape(self.ts.k, -1), s_sel
                ] = gc_significant[l].T
        elif self.test_type == "multivariate":
            warnings.warn(
    ...
    def __export_data(self):
        ...
        transfer_data = {
            ...
            "k": self.ts.k,
            "p": self.ts.p,
            "ts_names": self.ts.ts_names.tolist(),
            "cwd": self.cwd,
            "ts_combinations": self.ts_combinations.T.values.tolist()
            if self.test_type != "all_on_one"
            else [list(i) for i in self.ts_combinations.to_numpy().flatten()],
            "r_filename": self._file_names[2],
    ...
    def __call_r(self):
        subprocess.call(
            f"Rscript --vanilla main.R {self._file_names[0]} {self._file_names[1]}", shell=True, cwd=self.cwd
        )
    ...
    def __gen_conf_mat(self):
        ...
        for n, d in self.granger_results.items():
            ...
            self.granger_confusion_matrices[n] = TB.Tools.confussion_matrices(predictions=tmp_pred, actual=tmp_act)
    ...
    def __comp_multivariate_res(self):
        ...
        for n, d in self.granger_pvalues.items():
            self.multivariate_acc_approx[n] = [
                np.less(d[l], self.sig_lvl).astype(int).mean(axis=0) for l in np.arange(self.ts.p)
            ]
        self.approx_actual = [
            TB.Tools.t_inside(TB.Tools.rm_diag(self.ts.D))[: l + 1].mean(axis=0).mean(axis=0)
            for l in np.arange(self.ts.p)
        ]
    ...
    def plot_multi_comp(self, add=False, cwd=None, verbose=True):
        with sns.axes_style("darkgrid"):
            ...
            id = dict(zip(self.multivariate_acc_approx.keys(), np.arange(len(self.multivariate_acc_approx.keys()))))
            for i, ax in enumerate(axes.flat):
                for n, d in self.multivariate_acc_approx.items():
                    ax.plot(
                        x_ticks,
                        [d[t][i] for t in np.arange(self.ts.p)],
                        marker=self._marker[id[n] + 1],
                        linestyle="--",
                        alpha=0.5,
                        label=f"{n}",
                    )
                ax.plot(
                    x_ticks,
                    [t[i] for t in self.approx_actual],
                    label=f"Approximation of actual causal " f"influence on t{i + 1}",
                    marker="o",
                    alpha=0.75,
                )
                ax.legend(loc="best")
    ...
    def __calc_partial_acc(self):
        ...
        for l in np.arange(self.ts.p):
            tmp_cm_part = skm.confusion_matrix(
                y_pred=self.gc_partial_infl[l].flatten(),
    ...
    def plot_partial_comp(self, add=False, cwd=None, verbose=True):
        ...
        if not add:
            plt.suptitle(f"Accuracy of normal and partial bi-variate Granger-causality results for each lag", y=0.995)
        else:
            plt.suptitle(
                f"Accuracy of normal and partial bi-variate Granger-causality results for each lag {add})", y=0.995
            )
        ax = dict(zip(self.granger_confusion_matrices.keys(), axes.flat))
        with sns.axes_style("darkgrid"):
            for k, v in self.granger_confusion_matrices.items():
                for l in np.arange(self.ts.p):
                    not_part_lag[l] = np.divide(
                        np.trace(self.granger_confusion_matrices[k][l]), self.granger_confusion_matrices[k][l].sum()
                    )
                ax[k].plot(
                    _x_ticks,
                    self.part_acc_lag,
                    marker="o",
                    label="Accuracy of partial bi-variate GC",
                    alpha=0.5,
                    ls="solid",
                    lw=2,
                )
                ax[k].plot(
                    ...
                    not_part_lag,
                    marker="d",
                    label=f"Accuracy of normal bi-variate GC ({k})",
                    alpha=0.5,
                    ls="solid",
                    lw=2,
                )
    ...
    def plot_accuracy(self, verbose=True):
        for n, cm in self.granger_confusion_matrices.items():
            TB.Tools.plot_true_detections(
                confusion_matrices=cm,
                test_type=self.test_type,
                package=n,
                decay=self.ts.a,
                cwd=TB.Tools.create_dir_if_missing(dir=f"{self.fpath}/accuracy", return_dir=True),
                verbose=verbose,
            )
    ...
    def plot_correlations(self, verbose=True, sig_lvl=0.05, print_corr_summary=True):
        ...
        for n, d in self.granger_pvalues.items():
            pear, point = TB.Tools.corr_test(
                true_inf=TB.Tools.t_inside(TB.Tools.rm_diag(self.ts.D)),
                gc_pvalues=d,
            ...
            tmp_df = pd.DataFrame(
                [
                    [n, l + 1, pear[l][0][0], pear[l][0][1], point[l][0][0], point[l][0][1]]
                    for l in np.arange(self.ts.p)
                ],
                columns=col,
            )
    ...
    def plot_part_corr(self, verbose=True, sig_lvl=0.05):
        TB.Tools.corr_test(
            true_inf=TB.Tools.t_inside(TB.Tools.rm_diag(self.ts.D)),
            gc_pvalues=self.gc_partial_pvalues,
            true_coef=TB.Tools.t_inside(TB.Tools.rm_diag(self.ts.A)),
            sig_lvl=sig_lvl,
            package="Own implementation",
            decay=self.ts.a,
            cwd=TB.Tools.create_dir_if_missing(dir=f"{self.fpath}/correlation", return_dir=True),
            verbose=verbose,
            partial=True,
    ...
    def plot_conf(self, verbose=False):
        for n, d in self.granger_confusion_matrices.items():
            TB.Tools.plot_cm(
                confusion_matrices=d,
                cwd=TB.Tools.create_dir_if_missing(dir=f"{self.fpath}/confusion_matrices", return_dir=True),
                package_name=n,
                verbose=verbose,
            )
