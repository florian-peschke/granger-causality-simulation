...
class SensA:
    ...
    def run_gc(self):
        for i, v in enumerate(self.parameter_values):
            ...
            tmp_gc = Granger.Granger(
                test_type=self.test_type, significance_level=self.significance_level, cwd=self.cwd, ts_data=tmp_ts
            )
    ...
    def __calc_acc(self, conf, package, v, i):
        ...
        tmp_df.iloc[:, :] = [[package, np.divide(np.trace(c), np.sum(c)), i + 1, v] for i, c in enumerate(conf)]
        return tmp_df
    ...
    def __calc_correlations(self, pvalues, package, v, i):
        ...
        pb, pears = tb.Tools.calc_corr(
            true_inf=tb.Tools.t_inside(tb.Tools.rm_diag(self.ts_res[i].D)),
            gc_pvalues=pvalues,
            true_coef=tb.Tools.t_inside(tb.Tools.rm_diag(self.ts_res[i].A)),
            p=self.ts_res[i].p,
        )
        tmp_df.iloc[::2, :] = [
            [package, "Point-Biserial", pb[l][0][0], pb[l][0][1], l + 1, v] for l in np.arange(len(pb))
        ]
        tmp_df.iloc[1::2, :] = [
            [package, "Pearson", pears[l][0][0], pears[l][0][1], l + 1, v] for l in np.arange(len(pears))
        ]
        return tmp_df
    ...
    def plot_acc(self, verbose=True, chosen_lag=None):
        ...
        with sns.axes_style("darkgrid"):
            for n, p in self.df_acc.groupby("Package"):
                ...
                ax[n].plot(
                    tmp_mean, color=sns.color_palette("tab10")[1], label=r"$\mu$", marker="d", ls="dashed", alpha=0.2
                )
    ...
    def plot_corr(self, verbose=True, sig_lvl=0.05):
        ...
        with sns.axes_style("darkgrid"):
            for n, d in self.df_corr.groupby(["Package", "Type"]):
                for i, l in d.groupby("Lag"):
                    ...
                    tmp_p = l["p"].to_numpy()
                    ax[n].plot(
                        self.parameter_values, tmp_corr, marker=self._marker[int(i)], alpha=0.4, ls="--", color="k"
                    )
        ...
        plt.tight_layout()
        plt.savefig(
            fname=f"{self.fpath}/sens_corr_{self.parameter_name}_{str(datetime.datetime.now())}.pdf", format="pdf"
        )
        if verbose:
            plt.show()
        else:
    ...
    def plot_partial_comp(self, verbose=True):
        for i, o in enumerate(self.granger_objects):
            o.plot_partial_comp(
                add=f"({self.parameter_name} = {self.parameter_values[i]:.4f}",
